...
public class ResPublishSVCRegister implements BundleListener {
	...
	private void mappingResolvers(BaseResource res, DefinesRoot repo) {
		if (res.getResolverID() != null) {
			IPublishedFileResolver resolver = repo.getResResolvers().get(
					res.getResolverID());
	...
	private void initFiles(Object element, Object parent) {
		List<IConfigurationElement> ces = getConfigurationElements(element,
				"file");
	...
	private void initResolvers(IExtension root, DefinesRoot rootModel) {
		List<IConfigurationElement> ces = getConfigurationElements(root,
				"resolver");
		for (IConfigurationElement ce : ces) {
			try {
	...
	private IResourceVisitController getVisitController(Object parentEle) {
		List<IConfigurationElement> ces = getConfigurationElements(parentEle,
				"authority");
		for (IConfigurationElement ce : ces) {
			try {
	...
	private List<IConfigurationElement> getConfigurationElements(Object parent,
			String name) {
