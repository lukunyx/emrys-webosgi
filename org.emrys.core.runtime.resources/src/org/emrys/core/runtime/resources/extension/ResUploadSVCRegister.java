...
public class ResUploadSVCRegister implements BundleListener {
	...
	public static class ExtUploadReceiever {
		...
		public ExtUploadReceiever(String id, String name, IUploadFileReciever reciever, int priority) {
			this.id = id;
			this.name = name;
	...
	public List<ExtUploadReceiever> getSortedRegisteredRecievers(boolean forceUpdate) {
		if (internalUpdateMark || forceUpdate || recievers == null) {
			...
			IExtensionPoint extPoint = RegistryFactory.getRegistry().getExtensionPoint(
					RES_EXT_POINT_ID);
			IConfigurationElement[] cfEles = extPoint.getConfigurationElements();
			for (IConfigurationElement ele : cfEles) {
				...
				try {
					...
					ExtUploadReceiever repo = new ExtUploadReceiever(id, name, reciever, priority);
