...
public class DefaultLicenseProvider implements LicenseProvider {
	public final License getLicense(LicenseConsumer consumer) {
		...
		for (int i = 0; i < requiredLicenseTypes.length; i++) {
			List<String> supportedTypes = Arrays.asList(this
	...
	private License restoreLicense(LicenseType type) {
		...
		if (this instanceof ExternalLicenseProvider)
			licenseContent = ((ExternalLicenseProvider) this)
					.getLicenseContent();
		if (licenseContent == null) {
			try {
				licenseFile = LicenseManager.getInstance()
						.getLatestLicenseFile(type.getTypeName());
				if (licenseFile != null)
					licenseContent = FileUtil.getContent(licenseFile,
							"ISO8859-1").toString();
			} catch (Exception e) {
				// e1.printStackTrace();
			}
		...
		if (licenseFile != null && license instanceof DefaultLicense) {
			((DefaultLicense) license).setSourceFilePath(licenseFile
					.getAbsolutePath());
		}
		return license;
	}
				...
				try {
					...
					CommonActivator.getInstance().log(
							"License validate failed:"
									+ license.getInvalidCause(null), 0, false);
				} catch (Exception e) {
					// e.printStackTrace();
