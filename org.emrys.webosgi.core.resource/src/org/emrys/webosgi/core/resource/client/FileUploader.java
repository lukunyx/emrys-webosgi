...
public class FileUploader {
	...
	public boolean upload(IProgressMonitor monitor) throws Exception {
		...
		monitor.beginTask("Upload to web server: "
				+ new URI(context.getTargetUrl()).toString(), 100);
		PostMethod postMethod = new PostMethod(new URI(targetUrl.toString())
				.toString());
		try {
			...
			for (File f : files) {
				...
				for (NameValuePair param : fParams) {
					StringPart stringPart = new StringPart(param.getName(),
							param.getValue());
			...
			MultipartRequestEntity multReqEntity = new MultipartRequestEntity(
					params.toArray(new Part[params.size()]), postMethod
							.getParams());
	...
	/**
	 * If the target Server URL is just a file schema local url, just copy the
	 * uploding files to the target directory.
	 * 
	 * @throws Exception
	 */
