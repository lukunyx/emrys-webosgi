...
public class WarConnection extends AbstractConnection {
	 ...
	 */
	public WarConnection(final URL url, final Configuration config)
	...
	protected Properties getInstructions() throws MalformedURLException {
		...
		if (!instructions.containsKey("Import-Package")) {
			String packages = "javax.servlet,"
					...
					+ "org.apache.jasper.*;resolution:=optional," // extra
					// dependencies
					// for
					// JSP/JSF
					// War
			...
			// Certain Packages are always needed and therefore should be
			// appended.
			String importPackages = instructions.getProperty("Import-Package");

			if ((importPackages.contains(JAVAX_SERVLET))
					|| (importPackages.contains(JAVAX_SERVLET_HTTP))) {
				// found javax.servlet
				...
				for (String importstmt : imports) {
					if (importstmt.contains(JAVAX_SERVLET)) {
						if (importstmt.length() > JAVAX_SERVLET.length()
								&& importstmt.charAt(JAVAX_SERVLET.length()) == '.') {
			...
			if (!importPackages.contains(JAVAX_JSP)) {
				...
				importPackages += "," + JAVAX_JSP_JSTL
						+ ".*;resolution:=optional";
			}

			if (!importPackages.contains(APACHE_JASPER)) {
				importPackages += "," + APACHE_JASPER
						+ ".*;resolution:=optional";
			}

			if (!importPackages.contains(APACHE_TAGLIBS)) {
				importPackages += "," + APACHE_TAGLIBS
						+ ".*;resolution:=optional";
			}
		...
		if (getConfiguration().getImportPaxLoggingPackages()) {
			...
			importPackages += ",org.apache.commons.logging" + provider
					+ ",org.apache.commons.logging.impl" + provider
					+ ",org.apache.log4j" + provider + ",org.apache.log4j.spi"
					+ provider + ",org.apache.log4j.xml" + provider
					+ ",org.slf4j" + provider + ",org.slf4j.helpers" + provider
		...
		if (!instructions.containsKey("-removeheaders")) {
			instructions.setProperty("-removeheaders", "Private-Package,"
					+ "Ignore-Package");
		}
