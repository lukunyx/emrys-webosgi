...
/**
 * The java.net.URLClassLoader class allows one to load resources from arbitrary
 * URLs and in particular is optimized to handle "jar" URLs. Unfortunately for
 * jar files this optimization ends up holding the file open which ultimately
 * prevents the file from being deleted or update until the VM is shutdown.
 * 
 * The CloseableURLClassLoader is meant to replace the URLClassLoader and
 * provides an additional method to allow one to "close" any resources left
 * open. In the current version the CloseableURLClassLoader will only ensure the
 * closing of jar file resources. The jar handling behavior in this class will
 * also provides a construct to allow one to turn off jar file verification in
 * performance sensitive situations where the verification us not necessary.
 * 
...
public class CloseableURLClassLoader extends URLClassLoader {
	...
	// @GuardedBy("loaders")
	final ArrayList loaders = new ArrayList(); // package private to avoid
	// synthetic access.
	// @GuardedBy("loaders")
	private final ArrayList loaderURLs = new ArrayList(); // note: protected by
	// loaders
	// @GuardedBy("loaders")
	boolean closed = false; // note: protected by loaders, package private to
	...
	/**
	 * @param urls
	 *            the array of URLs to use for loading resources
	 * @see URLClassLoader
	 */
	 ...
	 *            the parent class loader used for delegation
	 * @param verifyJars
	 *            flag to determine if jar file verification should be performed
	 * @see URLClassLoader
	 */
	...
	private void checkForSealedPackage(Package pkg, String packageName, Manifest manifest,
			URL jarFileURL) {
	...
	/**
	 * The "close" method is called when the class loader is no longer needed
	 * and we should close any open resources. In particular this method will
	 * close the jar files associated with this class loader.
	 */
	public void close() {
		synchronized (loaders) {
			if (closed)
			...
			if (isFileJarURL(url)) {
				if (closed)
					throw new IllegalStateException(
							"Cannot add url. CloseableURLClassLoader is closed."); //$NON-NLS-1$
				loaderURLs.add(url);
				safeAddLoader(url);
				return;
			}
