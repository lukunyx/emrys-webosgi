...
public class DefaultLicenseType implements LicenseType {
	...
	public static void main(String[] args) throws Exception {
		...
		attrs.put(ATTR_NAME_CREATE_TIME, new SimpleDateFormat(TIME_FORMATE)
				.format(new Date()));
		attrs.put(ATTR_NAME_VALID_TIME, "900");
		attrs.put(ATTR_NAME_USER_NAME, "developer");
		attrs.put(ATTR_NAME_USER_MAIL, "developer@emrys.org");
		String encryptLicense = new DefaultLicenseType(null)
				.createNewLicense(attrs);
	...
	public DefaultLicenseType(LicenseProvider provider) {
		...
		attributes.put(ATTR_NAME_LICENSE_TYPE, new DefaultLicenseAttribute(
				ATTR_NAME_LICENSE_TYPE, "License Type", this.getTypeName(),
				"This attribute delicating the type of License", true, false,
				20));
		propNames.add(ATTR_NAME_LICENSE_TYPE);

		attributes.put(ATTR_NAME_CREATE_TIME,
				new DefaultLicenseAttribute(ATTR_NAME_CREATE_TIME,
						"Created Date", "",
						"The date of this license applied and created", true,
						false, 14));
		propNames.add(ATTR_NAME_CREATE_TIME);

		attributes.put(ATTR_NAME_ACTIVE_TIME, new DefaultLicenseAttribute(
				ATTR_NAME_ACTIVE_TIME, "Active Date", "",
		...
		attributes.put(ATTR_NAME_VALID_TIME, new DefaultLicenseAttribute(
				ATTR_NAME_VALID_TIME, "valid days", "90",
				"The Days befor expire", true, false, 7));
		propNames.add(ATTR_NAME_VALID_TIME);

		attributes.put(ATTR_NAME_USER_NAME, new DefaultLicenseAttribute(
				ATTR_NAME_USER_NAME, "User Name", "anonymous",
				"The user name who applied this license", true, false, 20));
		propNames.add(ATTR_NAME_USER_NAME);

		attributes.put(ATTR_NAME_USER_MAIL, new DefaultLicenseAttribute(
				ATTR_NAME_USER_MAIL, "User Email", "none",
				"The user email who applied this license", false, false, 50));
		propNames.add(ATTR_NAME_USER_MAIL);

		attributes.put(ATTR_NAME_MAX_ACTIVE_INTERVAL,
				new DefaultLicenseAttribute(ATTR_NAME_MAX_ACTIVE_INTERVAL,
						"Max Days before active", "7",
						"The user email who applied this license", false,
						false, 3));
		propNames.add(ATTR_NAME_MAX_ACTIVE_INTERVAL);

		attributes.put(ATTR_NAME_REQ_CODE, new DefaultLicenseAttribute(
				ATTR_NAME_REQ_CODE, "Applly Code", null, "Applly Code", true,
				false, 20));
	...
	public void setAttribute(StringBuffer rawLicense, String attrName,
			String value) {
	...
	public void validateLicense(License license) throws LicenseException {
		...
		if (license.getLicenseType().getTypeName().equals(this.getTypeName())) {
			...
			String validTimeStr = license
					.getAttributeValue(ATTR_NAME_VALID_TIME);
			...
			try {
				...
				if (validTimeStr.charAt(validTimeStr.length() - 1) == 'M')
					...
					try {
						validMonths = Integer.parseInt(validTimeStr.substring(
								0, validTimeStr.length() - 2));
					} catch (Throwable t) {
						throw new LicenseException(
								"Valid days or months of the license is illegal.");
					}
			...
			String createDateStr = license
					.getAttributeValue(ATTR_NAME_CREATE_TIME);

			Date createDate = null;
			try {
				createDate = new SimpleDateFormat(
						DefaultLicenseType.TIME_FORMATE).parse(createDateStr);
			} catch (ParseException e) {
				// e.printStackTrace();
				throw new LicenseException("The create date is invalid.");
			}

			String activeDateStr = license
					.getAttributeValue(ATTR_NAME_ACTIVE_TIME);

			String maxDaysBeforeActive;
			if (activeDateStr.length() == 0) {
				maxDaysBeforeActive = license
						.getAttributeValue(ATTR_NAME_MAX_ACTIVE_INTERVAL);
				...
				try {
					Date activeDate = new SimpleDateFormat(TIME_FORMATE)
							.parse(activeDateStr);
					...
					if (expireDate.getTime().before(new Date()))
						throw new LicenseException(
								"The license has been expired at "
										+ new SimpleDateFormat("yyyy/MM/dd")
												.format(expireDate.getTime()));
				} catch (ParseException e) {
					// e.printStackTrace();
					throw new LicenseException(
							"Invalid license for invalid active date.");
				}
			}
		} else {
			throw new LicenseException(
					"Wrong license type to valide by the current license type");
		}
	}
	...
	public String parseLicenseCode(String code) {
		...
		if (this.getTypeName().equals(
				getAttributeValue(ATTR_NAME_LICENSE_TYPE, decodeContent)))
	...
	public String createNewLicense(Map<String, String> attrs)
			throws LicenseException {
	 ...
	 * 
	 * @see
	 * org.emrys.webosgi.common.license.LicenseType#getLicenseDeclareContent ()
	 */
