...
class URLMapObjectSorter<T extends AbstMultiInstUrlMapObject> {
	 ...
	 * @param sortByPattern
	 *            whether to sort by the url pattern. The filter not need do so.
	 * @return the sorted cloned object for servlet or filter.
	 */
	public List<ClonedExecutableServletObject<T>> sort(
			Collection<? extends AbstMultiInstUrlMapObject> multiInstObjs,
			boolean sortByPattern, boolean forceUpdate) {
		if (buffer == null || forceUpdate) {
			...
			for (Iterator<? extends AbstMultiInstUrlMapObject> it = multiInstObjs
				...
				for (int i = 0; i < clonedInstances.length; i++) {
					buffer
							.add((ClonedExecutableServletObject<T>) clonedInstances[i]);
				}
			}
			// If need to sort by url map pattern.
			if (sortByPattern) {
	 ...
	 * @param <T0>
	 */
	class ReqServletMapComparator<T0 extends ClonedExecutableServletObject<T>>
			implements Comparator<T0> {
		/*
		 * (non-Javadoc)
		 * 
		 * @see java.util.Comparator#compare(java.lang.Object, java.lang.Object)
		 */
		public int compare(ClonedExecutableServletObject o1,
				ClonedExecutableServletObject o2) {

			String p1 = (o1.getOriginalObj()).getURLPatterns()[o1.getId()];
			String p2 = (o2.getOriginalObj()).getURLPatterns()[o2.getId()];
