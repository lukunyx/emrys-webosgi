...
public class FileUploadUtil {
	 ...
	 * @throws Exception
	 */
	public static void uploadFile(IUploadTaskContext context,
	...
	public static void uploadFile(final List<File> files, String uploadType,
			final String serverUrl, boolean keepFileStructure,
			IProgressMonitor monitor) throws Exception {
		Map<String, String> globalAttrs = new HashMap<String, String>();
		globalAttrs.put(IWebResConstants.FILE_PARA_TEYP, uploadType);
		globalAttrs.put(IWebResConstants.FILE_PARA_KEEP_FILE_STRUCTURE,
				keepFileStructure ? "true" : "false");
	 ...
	 * @throws Exception
	 */
	public static void uploadFile(final List<File> files,
			final Map<String, String> globalAttrs, final String serverUrl,
			IProgressMonitor monitor) throws Exception {
			...
			public List<NameValuePair> getParameters4File(File file) {
				...
				if (files.contains(file)) {
					result.add(new NameValuePair(
							IWebResConstants.FILE_PARA_CLIENT_ABSOLUTE_PATH,
							file.getAbsolutePath()));

				}
				result.add(new NameValuePair(
						IWebResConstants.FILE_PARA_TARGET_RELATIVE_PATH,
						getRelatedFilePath(file)));
				Iterator<Entry<String, String>> it = globalAttrs.entrySet()
						.iterator();
				while (it.hasNext()) {
					Entry<String, String> entry = it.next();
					result.add(new NameValuePair(entry.getKey(), entry
							.getValue()));
				}
				return result;
			}
			...
			private String getRelatedFilePath(File file) {
				if (files.contains(file)) {
					...
					if (parentFile != null) {
						...
						IPath relPath = FileUtil.makeRelativeTo(path, new Path(
								parentFile.getAbsolutePath()));
						return new Path(getRelatedFilePath(parentFile)).append(
								relPath).toPortableString();
					}
				}
