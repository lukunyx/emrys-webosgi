...
public class HttpSessionAdapter implements IServletObjectWrapper {
	...
	public static HttpSessionAdapter getAdapter(HttpSession session) {
		if (session != null) {
			for (HttpSessionAdapter w : wrappers) {
				if (session.equals((w).getOriginalObject())) {
					return w;
				}
			}
	...
	private final HttpSession wrapperedSession;
	 ...
	 * 
	 * @see
	 * org.eclipse.equinox.servletbridge.IServletObjectWrapper#getOriginalObject
	 * ()
	 */
