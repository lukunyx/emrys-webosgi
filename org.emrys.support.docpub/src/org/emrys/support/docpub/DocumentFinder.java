...
public class DocumentFinder {
	...
	public static String[] collectExtraDocuments(TocFile tocFile) {
		String dir = HrefUtil.normalizeDirectoryHref(tocFile.getID(),
				tocFile.getExtraDir());
		...
		if (url != null) {
			...
			result
					.addAll(collectExtraDocumentsFromZip(pluginID, directory,
							url));
		}
	...
	private static List collectExtraDocumentsFromZip(String pluginID,
	...
	private static List createExtraTopicsFromZipFile(String pluginID,
			ZipFile zipFile, String directory) {
		String constantHrefSegment = "/" + pluginID + "/"; //$NON-NLS-1$ //$NON-NLS-2$
		List result = new ArrayList();
		for (Enumeration entriesEnum = zipFile.entries(); entriesEnum
				.hasMoreElements();) {
