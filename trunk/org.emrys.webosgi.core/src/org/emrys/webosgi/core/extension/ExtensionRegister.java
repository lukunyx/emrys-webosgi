...
public class ExtensionRegister {
	public static class ExtReqProcessor {
		 ...
		 * @param priority
		 */
		public ExtReqProcessor(String id, String name,
				IReqProcessor preprocessor, int priority) {
			super();
			this.id = id;
			this.name = name;
	...
	private void parseReqPreprocessors() {
		IExtensionPoint extPoint = RegistryFactory.getRegistry()
				.getExtensionPoint(
						FwkActivator.getInstance().getBundleSymbleName()
								+ ".reqExtProcessor");
		...
		for (IConfigurationElement ce : ces) {
			...
			try {
				...
				IReqProcessor preprocessor;
				preprocessor = (IReqProcessor) ce
						.createExecutableExtension("class");
				ExtReqProcessor ext = new ExtReqProcessor(id, name,
