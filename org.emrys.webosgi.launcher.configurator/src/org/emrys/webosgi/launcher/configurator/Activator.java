 ...
 * {@link SimpleConfiguratorConstants#PROP_KEY_CONFIGURL} as a key. The value
 * will be used for the referal URL. Under the url, there must be a simple
 * bundles list file to be installed with thier start level and flag of marked
 * as started.
 * 
 * 2. If the value is null, do nothing any more. 3. Otherwise, retrieve the
 * bundles list from the url and install, set start level of and start bundles,
 * as specified.
 * 
...
public class Activator implements BundleActivator {
	...
	private org.ops4j.pax.url.war.internal.Activator wabUrlHanlderActivator;

	public static BundleContext INSTANCE;

	public void start(BundleContext context) throws Exception {
		INSTANCE = context;
		// Register the WAB url handler, ie. war, webbundle, wab.
		wabUrlHanlderActivator = new org.ops4j.pax.url.war.internal.Activator();
		wabUrlHanlderActivator.start(context);

		SimpleConfiguratorImpl bundleConfigurator = new SimpleConfiguratorImpl(
		...
		try {
			if (null != context.getBundle().loadClass(
				...
				commandRegistration = EquinoxUtils
		...
		if (DEBUG)
			System.out.println("Bundles Register Configuration completed.\n"); //$NON-NLS-1$
	}
	...
	public void stop(BundleContext context) throws Exception {
		...
		wabUrlHanlderActivator.stop(context);
	}
}
